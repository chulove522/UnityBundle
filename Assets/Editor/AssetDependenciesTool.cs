...
public class AssetDependenciesTool : EditorWindow {
    ...
    [MenuItem("MyTools/資源引用關系/查看直接引用", priority = 10)]
    public static void FindAssetsDependencies1() {
        recursiveFind = false;
        GetWindow<AssetDependenciesTool>(true, "查找資源引用關系-直接引用");
    }
    [MenuItem("MyTools/資源引用關系/查看遞歸引用", priority = 10)]
    ...
    private void OnGUI() {
        ...
        EditorGUILayout.LabelField("資源引用方數量 : " + prefabDependDic.Count);
        EditorGUILayout.LabelField("資源被引用方數量 : " + assetRelatedDic.Count);
        EditorGUILayout.LabelField("所統計的資源引用關系，不包含代碼動態加載");
    ...
    private void OnGUI_FindPrefab() {
        ...
        selectPrefabObject = EditorGUILayout.ObjectField("引用資源:", selectPrefabObject, typeof(Object), true, GUILayout.Width(600));
    ...
    private void OnGUI_FindAssets() {
        EditorGUILayout.BeginHorizontal();
        selectAssetObject = EditorGUILayout.ObjectField("被引用資源:", selectAssetObject, typeof(Object), true, GUILayout.Width(600));

        if (GUILayout.Button("分析被引用詳情", GUILayout.Width(100))) {
    ...
    private void OnGUI_AllPrefab() {
        ...
        EditorGUILayout.LabelField("搜索關鍵字（僅搜索當前頁！！！）:", GUILayout.Width(200));
        ...
        for (int i = showPageIndex1; i < showPageIndex1 + 100; i++) {
            ...
            EditorGUILayout.BeginHorizontal("box");
            EditorGUILayout.LabelField($"{i}. 資源文件名: {prefabPath}");
            EditorGUILayout.LabelField($"引用數量: {prefabDependDic[prefabPath].Length}", GUILayout.Width(200));
            if (GUILayout.Button("查看引用詳情", GUILayout.Width(100))) {
                prefabDependDic.TryGetValue(prefabPath, out var depPathArray);
        ...
        EditorGUILayout.BeginHorizontal();
        if (GUILayout.Button("首頁", GUILayout.Width(100))) {
            showPageIndex1 = 0;
        }
        if (GUILayout.Button("上一頁", GUILayout.Width(100))) {
            showPageIndex1 -= 100;
            if (showPageIndex1 < 0)
                showPageIndex1 = 0;
        }
        EditorGUILayout.LabelField($"{showPageIndex1}/{prefabNameArray.Length}", GUILayout.Width(100));
        if (GUILayout.Button("下一頁", GUILayout.Width(100))) {
            if ((showPageIndex1 + 100) < prefabNameArray.Length)
                showPageIndex1 += 100;
        }
        if (GUILayout.Button("尾頁", GUILayout.Width(100))) {
    ...
    private void OnGUI_AllAssets() {
        selectExtentIndex = EditorGUILayout.Popup("查找資源類型 : ", selectExtentIndex, assetExtentArray, GUILayout.MaxWidth(300));
        ...
        for (int i = showPageIndex2; i < showPageIndex2 + 100; i++) {
            ...
            EditorGUILayout.BeginHorizontal("box");
            EditorGUILayout.LabelField($"{i}. 資源文件名: {assetPath}");
            EditorGUILayout.LabelField($"被引用次數: {assetRelatedDic[assetPath].Length}", GUILayout.Width(200));
            if (GUILayout.Button("查看引用詳情", GUILayout.Width(100))) {
                assetRelatedDic.TryGetValue(assetPath, out var depPathArray);
    ...
    private void OnGUI_PrefabDetial(ref PrefabDependItem prefabDependItem) {
        ...
        if (prefabDependItem != null) {
            EditorGUILayout.BeginVertical("box");
            if (GUILayout.Button("關閉引用詳情", GUILayout.Width(100))) {
    ...
    private void FindAllPrefabDependencies(ref Dictionary<string, string[]> _prefabDependDic, ref Dictionary<string, string[]> _assetRelatedDic, out string[] _prefabNameArray, out string[] _assetExtentArray, out Dictionary<string, string[]> _assetExtentNameDic) {
        EditorUtility.DisplayProgressBar("讀取項目所有資源", "讀取所有資源", 0);
        ...
        EditorUtility.DisplayProgressBar("獲取資源引用關系", "遍歷所有資源", 0);
        for (int i = 0; i < allFilePathArray.Length; i++) {
            if (i % 100 == 0) {
                //间隔刷新UI提示框
                if (EditorUtility.DisplayCancelableProgressBar("獲取資源引用關系", $"遍歷所有文件 : {i}/{allFilePathArray.Length}", (float)i / allFilePathArray.Length))
                    break;
            }
            _prefabDependDic[allFilePathArray[i]] = AssetDatabase.GetDependencies(allFilePathArray[i], recursiveFind);
        }

        EditorUtility.DisplayProgressBar("讀取項目所有資源", "資源引用排序", 0);
        ...
        EditorUtility.DisplayProgressBar("反向解析引用關系", "遍歷所有資源", 0);

        Dictionary<string, List<string>> tempAssetDic = new Dictionary<string, List<string>>();
        for (int i = 0; i < allFilePathArray.Length; i++) {
            if (i % 200 == 0) {
                //间隔刷新UI提示框
                if (EditorUtility.DisplayCancelableProgressBar("反向解析引用關系", $"遍歷所有文件 : {i}/{allFilePathArray.Length}", (float)i / allFilePathArray.Length))
                    break;
            }
        ...
        EditorUtility.DisplayProgressBar("反向解析引用關系", "資源引用排序", 0);
        ...
        EditorUtility.DisplayProgressBar("讀取引用資源類型", "獲取所有引用文件類型", 0);
    ...
    public class PrefabDependItem {
        ...
        private void LoadDependObjects() {
            ...
            for (int i = 0; i < dependPathArray.Length; i++) {
                if (EditorUtility.DisplayCancelableProgressBar("加載引用該資源的Prefab|Scene", $"加載進度 : {i}/{dependPathArray.Length}", (float)i / dependPathArray.Length)) {
                    EditorUtility.ClearProgressBar();
                    return;
                }
    ...
    public class AssetRelatedItem {
        ...
        private void LoadRelatedObjects() {
            ...
            for (int i = 0; i < relatedPathArray.Length; i++) {
                if (EditorUtility.DisplayCancelableProgressBar("加載引用該資源的Prefab|Scene", $"加載進度 : {i}/{relatedPathArray.Length}", (float)i / relatedPathArray.Length)) {
                    EditorUtility.ClearProgressBar();
                    return;
                }
